...
class Login2Activity : AppCompatActivity() {
    ...
    private fun login() {
        ...
        showSimpleProgressDialog(this@Login2Activity, null, "Loading...", false)
    ...
    private fun onTaskCompleted(response: String, task: Int) {
        ...
        when (task) {
            LoginTask -> if (isSuccess(response)) {
                saveInfo(response)
                Toast.makeText(this@Login2Activity, "Login Successfully!", Toast.LENGTH_SHORT).show()
                val intent = Intent(this@Login2Activity, WelcomeActivity::class.java)
                intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK or Intent.FLAG_ACTIVITY_NEW_TASK)
                startActivity(intent)
                this.finish()
            } else {
                Toast.makeText(this@Login2Activity, getErrorMessage(response), Toast.LENGTH_SHORT).show()
            }
        }
    }
